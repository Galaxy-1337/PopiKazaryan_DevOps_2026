...
| 403 | `forbidden` | у учётной записи нет права, требуемого методом |
...
### `GET /api/v1/auth/roles` — права учётных записей

Доступен без входа. Возвращает массив учётных записей с перечнем доступных им действий.
В системе одна учётная запись — организатор:
...
Учётная запись организатора имеет полный доступ к функционалу приложения:
...
| `thesis:submit_own` | подача тезисов |
| `user:manage` | управление учётными записями |

Права вида `*_own` означают работу со своими данными. Организатор обладает и ими, и правами
`*_all`, поэтому видит и изменяет данные всех участников конференции.
...
Недостаток прав → `403 forbidden`; в сообщении указывается учётная запись и требуемое право,
например: `Учётная запись «no-permissions@example.com» не имеет права на действие
«conference:manage»`.

### Доступ к данным

- **Конференции, секции, реестр участников.** Чтение доступно любой вошедшей учётной записи
  (нужно для выбора участника и просмотра списков); изменение требует прав
  `conference:manage` / `section:manage` / `participant:manage`.
- **Заявки.** Право `application:read_all` показывает все заявки, `application:decide`
  позволяет принимать по ним решения. Без `*_all` выборка фильтруется по участнику,
  связанному с учётной записью; обращение к чужой заявке → `403 forbidden_not_owner`,
  попытка создать заявку от имени другого участника → `403 forbidden_foreign_participant`,
  отсутствие связанного участника → `403 no_participant_profile`.
- **Оргвзносы.** Право `fee:manage` открывает все взносы; без него видны только взносы по своим
  заявкам, чужой взнос → `403 forbidden_foreign_fee`.
- **Приглашения.** Право `invitation:manage` управляет очередью рассылки; без него видны только
  адресованные учётной записи приглашения.
- **Тезисы.** Право `thesis:review` позволяет оценивать тезисы, `thesis:read_all` — видеть все,
  `thesis:submit_own` — подавать свои. Обращение к чужим тезисам → `403 forbidden_foreign_thesis`.
- **Гостиница.** Право `hotel:manage` управляет всеми бронями и подтверждает их; без него
  доступны только свои брони, чужая → `403 forbidden_foreign_booking`.
- **Отчёты** (`/api/v1/reports/*`). Требуется право `report:read`.
...
Учётная запись создаётся при старте приложения, если включено наполнение демонстрационными
данными (`SEED_DEMO_DATA=true`). Пароль задаётся переменной `DEMO_PASSWORD`; явно заданный
`ADMIN_PASSWORD` имеет приоритет. Других учётных записей в системе нет: категории участников
(слушатель, докладчик, рецензент) хранятся только как характеристика записи в реестре
участников и входа в систему не имеют.
...
curl -s http://127.0.0.1:8000/api/v1/reports/sections-load/1 -H "Authorization: Bearer $TOKEN"
```
...
### `GET /api/v1/reports/sections-load/{conference_id}` — заполненность секций

Показывает по каждой секции вместимость, число занятых мест и остаток. Отчёт нужен
программному комитету для планирования: видно, какие секции заполнены, где есть свободные
места и не переполнена ли секция после изменения вместимости.

```bash
curl -s http://127.0.0.1:8000/api/v1/reports/sections-load/1
```
...
* место занимают заявки в статусах `submitted` и `accepted` — ровно те же статусы, что
  использует правило вместимости секции (`ACTIVE_SECTION_STATUSES` в `app/services.py`)
  при приёме новой заявки; поэтому отчёт и правило всегда согласованы;
* отклонённые (`rejected`) и отозванные (`withdrawn`) заявки места не занимают;
* переполнение секции через API недостижимо: изменение вместимости ниже числа занятых мест
  отклоняется правилом `section_capacity_below_load`. Поэтому `free_seats` не бывает
  отрицательным, а `is_full` принимает значение `true`, когда занято не меньше вместимости;
* если у конференции нет секций, отчёт возвращается с нулевыми итогами и пустым списком
  `items`.

Чем отличается от `GET /api/v1/sections`: список секций отдаёт заполненность по одной секции и
выполняет отдельный запрос на каждую секцию, а отчёт даёт сводку по конференции целиком —
итоги по вместимости, занятым и свободным местам, процент заполненности — за два запроса к
базе. Кроме того, список секций доступен любому вошедшему пользователю, а отчёт — только
учётной записи с правом `report:read`.
...
| `/` | рабочая страница со вкладками (состав зависит от прав учётной записи); без сессии перенаправляет на `/login` (HTTP 303) |
| `/static/css/app.css` | стили |
| `/static/js/app.js` | клиентская логика вызовов API |

Состав вкладок зависит от прав учётной записи: разделы, на которые права нет, не отображаются,
а серверные методы всё равно проверяют право при каждом запросе.
Организатору доступны семь вкладок: Сводка, Заявки, Оргвзносы, Приглашения, Гостиница, Тезисы,
Участники.
