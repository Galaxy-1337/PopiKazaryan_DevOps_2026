...
- [ ] покрыто тестами, `.\scripts\dev.ps1 verify` зелёный

## Как проверять
`.\scripts\dev.ps1 run` → вкладка «Гостиница» → подтвердить бронь
```
...
Команда выполняет `ruff format --check`, `ruff check`, `pytest` и дымовую проверку
